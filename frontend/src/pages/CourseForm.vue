...
const courseCreationResource = createResource({
  ...
  makeParams(values) {
    const filteredValues = filterLegacyFields(values)
    const docData = {
      doctype: 'LMS Course',
    ...
    const doc = filterLegacyFields(docData)
    return { doc }
  },
})
...
const courseEditResource = createResource({
  ...
  auto: false,
  makeParams(values) {
    const filteredCourse = filterLegacyFields(course)
    const fieldname = filterLegacyFields({
      image: course.course_image?.file_url || '',
      instructors: instructors.value.map((instructor) => ({
        instructor: instructor,
      })),
...
const courseResource = createResource({
  ...
  onSuccess(data) {
    Object.keys(data).forEach((key) => {
      ...
      } else if (Object.hasOwn(course, key) && !LEGACY_FIELDS.includes(key)) {
        course[key] = data[key]
      }
    })
    let checkboxes = [
